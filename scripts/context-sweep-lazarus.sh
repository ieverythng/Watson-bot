...
#   bash scripts/context-sweep-lazarus.sh --dry-run
# Live sweep (will restart llama.cpp/LiteLLM for each context):
#   bash scripts/context-sweep-lazarus.sh --live --contexts 80000,96000,112000,128000
...
PY
  ...
  if [[ "$DRY_RUN" -eq 1 ]]; then
    bash "$ROOT/scripts/lazarus.sh" --dry-run --mode "$MODE" --ctx "$ctx" | tee "$lazarus_log"
    lazarus_status="dry-run"
    harness_report="skipped"
  else
    if bash "$ROOT/scripts/lazarus.sh" --mode "$MODE" --ctx "$ctx" | tee "$lazarus_log"; then
      lazarus_status="ok"
      harness_log="$OUT_DIR/harness-${label}.log"
      python3 "$ROOT/scripts/watson-stack-harness.py" \
        --base-url "$BASE_URL" \
